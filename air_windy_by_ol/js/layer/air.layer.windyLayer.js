...
var WindyLayer = function (_ol$layer$Image) {
  ...
  WindyLayer.prototype.appendTo = function appendTo(map) {
    if (map && map instanceof ol.Map) {
      ...
      map.on('pointermove', evt => {
        if(this.data){
          var r = this.getWindyValue(evt.coordinate)
          overlay.setPosition(evt.coordinate)
          infoDiv.innerText = `风向:${r.direction} 风速:${Math.round(r.speed) * 3.6}km/h`
        }
      })
    } else {
  ...
  WindyLayer.prototype.getWindyValue = function getWindyValue(point) {
    var lng = Math.round(point[0])
    var lat = Math.round(point[1])
    var index = 360 * (90 - lat) + 180 + lng 
    var xValue = this.data[1].data[index]
    var yValue = this.data[2].data[index]
    var angle = this.getWindAngle(xValue,yValue)
    var direction = this.getWindyDirection(angle)
    var speed = Math.sqrt(xValue * xValue + yValue * yValue)
    return {
      direction: direction,
      angle: angle,
      speed: speed,
  ...
  WindyLayer.prototype.getWindAngle = function getWindAngle (u, v) {
  ...
  WindyLayer.prototype.getWindyDirection = function getWindyDirection(angle) {
    let direction = ''
